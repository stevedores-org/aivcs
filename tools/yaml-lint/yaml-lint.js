...
var TRUTHY_VALUE = /^(?:yes|no|on|off)$/i;
var BLOCK_SCALAR_TYPES = new Set(["BLOCK_LITERAL", "BLOCK_FOLDED"]);
function blockScalarLines(docs, lineCounter2) {
  const skip = new Set;
  for (const doc of docs) {
    $visit(doc, {
      Scalar(_key, node) {
        if (!node.range || !BLOCK_SCALAR_TYPES.has(node.type ?? ""))
          return;
        const start = lineCounter2.linePos(node.range[0]);
        const end = lineCounter2.linePos(node.range[1]);
...
function checkTabs(file, lines, skip) {
  const findings = [];
...
function checkIndentSpacing(file, lines, skip) {
  const findings = [];
  lines.forEach((line, i) => {
    if (skip.has(i + 1))
      return;
...
function checkTruthy(file, lines, skip) {
  const findings = [];
  lines.forEach((line, i) => {
    if (skip.has(i + 1))
      return;
...
function checkCommentSpacing(file, lines, skip) {
  const findings = [];
  lines.forEach((line, i) => {
    if (skip.has(i + 1))
      return;
...
function checkSecretShapes(file, lines) {
  const findings = [];
  lines.forEach((line, i) => {
...
function checkSyntax(file, docs, lineCounter2) {
  const findings = [];
  for (const doc of docs) {
    for (const err of doc.errors) {
      const pos = err.pos?.[0] ?? 0;
      const { line, col } = lineCounter2.linePos(pos);
...
`);
  ...
  const docs = $parseAllDocuments(text, { lineCounter: lineCounter2 });
  const skip = blockScalarLines(docs, lineCounter2);
  return [
    ...checkSyntax(file, docs, lineCounter2),
...
// index.ts
var DEFAULT_ROOTS = ["crossplane", "flux", "clusters", "k8s", ".github/workflows"];
var ROOTS = process.env.YAML_LINT_ROOTS ? process.env.YAML_LINT_ROOTS.split(",").map((r) => r.trim()).filter(Boolean) : DEFAULT_ROOTS;
var ENFORCE = process.env.YAML_LINT_ENFORCE !== "false";
var IGNORE_GLOBS = [
  "**/upstream/**",
  "**/cert-manager-*.yaml",
...
async function main() {
  ...
  const errors2 = findings.filter((f) => f.severity === "error");
  const warnings = findings.filter((f) => f.severity === "warning");
  for (const f of [...errors2, ...warnings]) {
    console.log(annotate(f));
  }
  console.log(`
yaml-lint: scanned ${files.length} file(s) \u2014 ${errors2.length} error(s), ${warnings.length} warning(s)` + (!ENFORCE && errors2.length > 0 ? " (warn mode \u2014 not failing the build)" : ""));
  return ENFORCE && errors2.length > 0 ? 1 : 0;
}
