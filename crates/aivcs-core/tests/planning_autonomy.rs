...
use aivcs_core::{
    ...
    schedule_next_ready_tasks, EpicPlan, GoalPlan, PlanTask, PlanTaskStatus, PlannerRuntimeState,
    RecoveryControls, ReplanPolicy, ReplanSuppressionReason, SchedulerConstraints, TaskPlan,
};
...
#[test]
fn recovery_controls_suppress_replan_during_cooldown() {
    let now = Utc::now();
    let mut tasks = BTreeMap::new();
    let mut t1 = PlanTask::pending("t1", vec![], now - Duration::hours(30));
    t1.status = PlanTaskStatus::Failed {
    ...
    let controls = RecoveryControls {
        ...
        max_replans_per_window: 5,
        max_consecutive_failed_replans: 3,
    };
    let runtime = PlannerRuntimeState {
        last_replan_at: Some(now - Duration::hours(1)),
        replans_in_window: 1,
        consecutive_failed_replans: 0,
    };

    let decision = evaluate_replan_with_controls(&dag, &policy, &controls, &runtime, now);
    assert!(!decision.should_replan);
    assert!(matches!(
        decision.suppressed_by,
        Some(ReplanSuppressionReason::CooldownActive { .. })
    ));
}

#[test]
fn recovery_controls_suppress_replan_on_window_limit() {
    let now = Utc::now();
    let mut tasks = BTreeMap::new();
    let mut t1 = PlanTask::pending("t1", vec![], now - Duration::hours(24));
    t1.status = PlanTaskStatus::Blocked {
        reason: "dependency outage".to_string(),
    };
    t1.confidence = 0.4;
    tasks.insert("t1".to_string(), t1);
    ...
    let policy = ReplanPolicy {
        min_confidence: 0.6,
        max_blocked_ratio: 0.1,
        trigger_on_failure: true,
        max_stale_hours: 12,
    };
    let controls = RecoveryControls {
        replan_cooldown_hours: 0,
        max_replans_per_window: 2,
        max_consecutive_failed_replans: 3,
    };
    let runtime = PlannerRuntimeState {
        last_replan_at: None,
    ...
    assert!(matches!(
        decision.suppressed_by,
        Some(ReplanSuppressionReason::WindowLimitReached { .. })
    ));
}

#[test]
fn recovery_controls_suppress_replan_on_consecutive_failures() {
    let now = Utc::now();
    let mut tasks = BTreeMap::new();
    let mut t1 = PlanTask::pending("t1", vec![], now - Duration::hours(24));
    t1.status = PlanTaskStatus::Failed {
        reason: "runtime crash".to_string(),
    };
    t1.confidence = 0.3;
    tasks.insert("t1".to_string(), t1);
    ...
    let controls = RecoveryControls {
        replan_cooldown_hours: 0,
        max_replans_per_window: 10,
        max_consecutive_failed_replans: 2,
    };
    let runtime = PlannerRuntimeState {
        last_replan_at: None,
        replans_in_window: 1,
        consecutive_failed_replans: 2,
    };
    ...
    assert!(matches!(
        decision.suppressed_by,
        Some(ReplanSuppressionReason::ConsecutiveFailuresExceeded { .. })
    ));
}

#[test]
fn recovery_controls_allow_replan_when_bounds_are_respected() {
    let now = Utc::now();
    let mut tasks = BTreeMap::new();
    let mut t1 = PlanTask::pending("t1", vec![], now - Duration::hours(30));
    t1.status = PlanTaskStatus::Blocked {
    ...
    let controls = RecoveryControls::default();
    let runtime = PlannerRuntimeState {
        last_replan_at: Some(now - Duration::hours(8)),
        replans_in_window: 1,
        consecutive_failed_replans: 0,
    };
    ...
    let decision = evaluate_replan_with_controls(&dag, &policy, &controls, &runtime, now);
    assert!(decision.should_replan);
    assert!(decision.suppressed_by.is_none());
}
