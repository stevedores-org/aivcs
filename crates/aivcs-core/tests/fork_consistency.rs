...
async fn test_fork_consistency_and_uniqueness() {
    ...
    // 2. Fork
    let result = fork_agent_parallel(Arc::clone(&handle), &parent_id.hash, 2, "fork").await.unwrap();
    ...
    for commit_id in result.commit_ids {
        ...
        if commit_id.hash != expected_id.hash {
            ...
            println!("Stored state: {}", serde_json::to_string(&snapshot.state).unwrap());
            println!("Expected hash: {}", expected_id.hash);
            println!("Actual hash:   {}", commit_id.hash);
        }

        assert_eq!(commit_id.hash, expected_id.hash, "Commit ID must match canonical hash of stored state");
    }
}
...
async fn test_fork_with_non_object_state() {
    ...
    // 2. Fork
    let result = fork_agent_parallel(Arc::clone(&handle), &parent_id.hash, 1, "fork").await.unwrap();
