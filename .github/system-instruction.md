<!-- GENERATED by `six-files compile` — do not hand-edit. Sources: AGENTS.md, intent/current.yaml. Re-run the compiler to update. -->

<!-- Consumer: Antigravity / Google AI Studio (`.github/system-instruction.md`) -->

# six-files — System Instruction

## Active intent

**Objective:** `OBJ-2026-SDF-002` — Dual ledger — link GitHub PRs to aivcs CommitId
**Updated:** 2026-06-08T12:00:00+00:00
**Tracking:** issue #220 on branch `feat/sdf-intent-pilot`
...
## Agent capabilities (from AGENTS.md)
...
**Run `og-crab run` before pushing any PR.** This is the canonical pre-push gate for every contributor and every coding agent that touches this repo. Configuration lives in [`propel.toml`](propel.toml); the binary comes from [lornu-ai/og-crab](https://github.com/lornu-ai/og-crab).

```bash
og-crab run              # run all enabled checks from propel.toml
og-crab run --skip-cache # force re-run, ignoring cached results
og-crab run fmt clippy   # run a subset of checks
```

Install once with `cargo install --git https://github.com/lornu-ai/og-crab og-crab`. Cache state in `.og-crab/cache/` means repeat runs only re-execute checks whose inputs changed.

**The rule:** if `og-crab run` is red, do not push. Either fix the failure or, with a documented reason, disable the check in `propel.toml`. **Do not** push a known-red PR with the expectation that GitHub CI will surface the same failure — that wastes reviewer time and burns CI minutes on issues you could have caught in seconds locally.

**Why this is mandatory:** the most common reason a PR sits BLOCKED on this repo is a fmt/clippy/test failure that `og-crab` would have caught before push. Agents that bypass this gate produce PRs that consume team review bandwidth on self-inflicted issues. This rule applies equally to humans and to coding agents (Claude, Codex, Cursor, Copilot, Jules, Antigravity).

#### Fallbacks (only when `og-crab` is unavailable)

If `og-crab` cannot be installed for some reason, fall back to one of these — but the gate is still the same set of checks (fmt + clippy + test + yaml validate):
...
|------|-------------------|-------------------|
| **Pre-push validation (MANDATORY)** | **`og-crab run`** | `just validate-all` / `make test` |
| Pre-push validation (uncached) | `og-crab run --skip-cache` | — |
| Run one check | `og-crab run clippy` (or `fmt`, `test`, …) | — |
| Rust fmt check | `og-crab run fmt` | `cargo fmt --check` |
| Rust clippy | `og-crab run clippy` | `cargo clippy -- -D warnings` |
| Rust tests | `og-crab run test` | `cargo test --all-features` |
